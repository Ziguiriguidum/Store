		...
		<div class="sm:flex flex-wrap mx-16 pb-8 justify-around">
			{#each games as game}
				<button
					...
					class="pb-4 hover:scale-125 hover:text-white brightness-90 hover:brightness-110 cursor-pointer transition p-2 hover:bg-[#1f1f1f] hover:z-50 rounded"
				>
					<img
						width="256px"
						...
						alt="icon"
					/>
					<p class="text-gray-400 text-xs w-64 whitespace-nowrap text-ellipsis">
						{game.name}{game.version !== null
							? ' - ' + game.version.substring(0, 10).replace(/\.$/, '...')
							: ''} - {Number(game.size).toLocaleString()}GB
					</p>
				</button>
			{/each}
		</div>
