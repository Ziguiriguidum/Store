...
{#if props.installGame}
	...
	>
		<button
			...
			class="border border-gray-400 cursor-default border-opacity-30 bg-[#0a0a0a] p-2 text-gray-400 rounded-lg w-96 px-4"
		>
			<div class="flex border-b border-white border-opacity-30 mb-2 pb-1 text-left">
				...
				<button
					class="float-right hover:brightness-125"
					on:click={() => (props.installGame = null)}
				>
					X
				</button>
			</div>
			<button
				on:click={() =>
					open({ directory: true, defaultPath: installPath || 'C:\\' }).then((e) => {
						e ? (installPath = e.toString()) : '';
					})}
				class="absolute translate-x-80 brightness-[10] hover:brightness-[3] cursor-pointer"
				>📁</button
			>
			<input
				type="text"
				bind:value={installPath}
				class="w-full bg-[#131313] pl-4 rounded-lg text-lg"
				placeholder="Install Path"
			/>
			<div class="flex text-left">
				<p class="text-xs w-10/12 px-2">
					You need at least {props.installGame.internal.size.toLocaleString()}GB available to
					perform this action!
				</p>
				<button
